...
{
    public interface IInteractionYoutube : IInteractionOpenMedia, interactions.IInteraction, INotifyPropertyChanged
    {
        IInteractionWorkItem InteractionWorkItem { get; }
    }

    public class YoutubeParty : InteractionParty, IInteractionParty, INotifyPropertyChanged
    {
        private IInteractionParty InteractionParty { get; set; }

        public YoutubeParty()
        {
        }

        public YoutubeParty(IInteractionParty interactionParty, Genesyslab.Desktop.Modules.Core.Model.Interactions.IInteraction interaction)
        {
            this.InteractionParty = interactionParty;
            this.FormattedObject = this.InteractionParty.FormattedObject;
            this.UserData = this.InteractionParty.UserData;
            this.InteractionPartyType = this.InteractionParty.InteractionPartyType;
            this.RelatedInteraction = interaction;
        }

        public override string ToString()
        {
            return string.Format("Name:'{0}' DisplayName:'{1} 'Type:'{2}'", (object)this.Name, (object)this.FormattedObject.FormattedString, (object)this.InteractionPartyType.ToString());
        }
    }


    public class InteractionYoutube :
         InteractionOpenMedia, IInteractionYoutube, IInteractionOpenMedia, interactions.IInteraction, INotifyPropertyChanged
    {
        ...
        public InteractionYoutube(IUnityContainer container, IInteractionManager interactionManager,
            ...
            : base(container, interactionManager, entrepriseService, agent, configManager, configAttachedDataManager)
        {
            ...
            this.log.Info((object)"InteractionYoutube()");
            ...
            (this.InteractionWorkItem.InteractionParties as INotifyCollectionChanged).CollectionChanged +=
                new NotifyCollectionChangedEventHandler(this.InteractionYoutube_CollectionChanged);
        }
        ...
        {
            (this.InteractionWorkItem.InteractionParties as INotifyCollectionChanged).CollectionChanged -= new NotifyCollectionChangedEventHandler(this.InteractionYoutube_CollectionChanged);
        ...
        private void InteractionWorkItem_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            try
            {
                if (InteractionYoutube.NoUpdateProperties.Contains(e.PropertyName))
                    return;
                ...
                if (this.log.IsDebugEnabled && InteractionYoutube.TraceProperties.Contains(e.PropertyName))
                {
                    StringBuilder possibleStringBuilder = new StringBuilder();
                    foreach (string traceProperty in (IEnumerable<string>)InteractionYoutube.TraceProperties)
        ...
        private void InteractionYoutube_CollectionChanged(
          object sender,
